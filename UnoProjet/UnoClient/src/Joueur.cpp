...
void Joueur::supprimerCarteJoueur(Carte c){
  ...
  for(int i=0;i<(int)_cartesJoueur.size();i++){
    if((_cartesJoueur[i].getValeur() == c.getValeur()) && (_cartesJoueur[i].getCouleur() == c.getCouleur()) || ((c.getValeur()==44 && _cartesJoueur[i].getValeur()==44) || (c.getValeur()==11 && _cartesJoueur[i].getValeur()==11))){
      pos=i;
      break;
    }
  }
