...
void VueGraphique::run(){
  ...
  jeton.setOutlineColor(Color(250, 150, 100));
    ...
    while (window.pollEvent(event)){
		int id;
		if(_client.envoyerCarteCourante().getValeur()!=0) id=_client._idJoueurCourant;
		id=_client._id-id;
	
		if(id==1) jeton.setPosition(X/2,y);
...
void VueGraphique::runEnd(){
    ...
    RectangeShape fin((Vector2f(2*X/3,2*Y/3));
	
	if(_client._vainqueur==login){
		fin.setTexture(&winner);
	}else{
		fin.setTexture(&looser);
	}
