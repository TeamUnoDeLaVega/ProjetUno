...
public:
  ...
  sf::TcpListener listener;
		...
		void bloquerSocket();
		Carte recevoirCarteANouveau();
		void piocher(Joueur *j);
		int peutJouer(Carte c);

		void envoyerVainqueur(std::string nomVainqueur);
		void piocherNom(std::string nomJoueur);
		void envoyerJoueurCourant(int idJoueurCourant);
		void envoyerId(Joueur *j,int id);
		void envoyerNbCartes();
