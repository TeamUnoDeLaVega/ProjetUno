...
int Jeu::getJoueurCourant() const {
 ...
 void Jeu::historique(Carte c){
  ...
  for(int i=0;i<_historique.size();i++) std::cout << _historique[i].getValeur() << _historique[i].getCouleur() ;
 }
...
void Jeu::supprimerCarteJoueur(Joueur* j,Carte c){
  ...
  if((j->_cartesJoueur[i].getValeur() == c.getValeur()) && (j->_cartesJoueur[i].getCouleur() == c.getCouleur()) || ((c.getValeur()==44 && j->_cartesJoueur[i].getValeur()==44) || (c.getValeur()==11 && j->_cartesJoueur[i].getValeur()==11))){
      pos=i;
      break;
    }
  }
...
void Jeu::remplirPaquet(){
  for(int i=0;i<(int)_historique.size();i++){
    _paquet.push_back(_historique[i]);
  }
   for(int i=0;i<(int)_historique.size();i++){
