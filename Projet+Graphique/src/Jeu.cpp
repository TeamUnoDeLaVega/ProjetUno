...
void Jeu::creerJoueur(){
  ...
  Joueur j4(4);
   ...
   _players.push_back(j1);
   _players.push_back(j2);
   _players.push_back(j3);
   _players.push_back(j4);
}
...
int Jeu::tailleJoueur(int i)const{
  std::vector<Carte> pl=_players[i].getCartesJoueur();
  return pl.size();
}
